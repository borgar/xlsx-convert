...
var import_simple_xml10 = require("@borgar/simple-xml");
...
// src/handler/rels.ts
var import_simple_xml = require("@borgar/simple-xml");
function handlerRels(dom, basepath = "xl/workbook.xml") {
  basepath = import_path.default.dirname(basepath);
  const rels = [];
  if (dom) {
    dom.querySelectorAll("Relationship").forEach((d) => {
      ...
      for (const p of REL_PREFIXES) {
        if (type.startsWith(p)) {
          ...
          if (mode !== "External") {
            target = import_path.default.join(basepath, target);
          }
          break;
        }
      }
...
var import_simple_xml2 = require("@borgar/simple-xml");

// src/utils/normalizeFormula.ts
var import_fx = require("@borgar/fx");
...
function normalizeFormula(formula, wb) {
  const tokens = (0, import_fx.tokenize)(formula.normalize(), { xlsx: true });
  let normalized = "";
  tokens.forEach((t) => {
    if ((0, import_fx.isFunction)(t)) {
      normalized += t.value.replace(/^(?:_xlfn\.|_xludf\.|_xlws\.)+/i, "");
      return;
    } else if ((0, import_fx.isReference)(t) && wb?.externalLinks) {
      if (t.type === import_fx.tokenTypes.REF_NAMED) {
        t.value = t.value.replace(/^(?:_xlpm\.)/ig, "");
      }
      if (t.value.includes("[")) {
        let newValue;
        if (t.type === import_fx.tokenTypes.REF_STRUCT) {
          const ref = (0, import_fx.parseStructRef)(t.value, { xlsx: true });
          if (ref.table && wb.tables?.length) {
          }
          ref.context = updateContext(ref, wb.externalLinks);
          newValue = (0, import_fx.stringifyStructRef)(ref);
        } else {
          const ref = (0, import_fx.parseA1Ref)(t.value, { xlsx: true });
          ref.context = updateContext(ref, wb.externalLinks);
          newValue = (0, import_fx.stringifyA1Ref)(ref);
        }
        normalized += newValue;
        return;
      }
...
var import_simple_xml3 = require("@borgar/simple-xml");
...
var import_simple_xml4 = require("@borgar/simple-xml");
...
var Color = class {
  ...
  rgb() {
    return (0, import_d3_color.rgb)(this.r, this.g, this.b);
  }
  hsl() {
    return (0, import_d3_color.hsl)(this.rgb());
  }
  toJSON() {
...
var import_simple_xml5 = require("@borgar/simple-xml");
...
var import_simple_xml6 = require("@borgar/simple-xml");
...
var import_simple_xml7 = require("@borgar/simple-xml");
...
var import_fx4 = require("@borgar/fx");
...
// src/handler/cell.ts
var import_numfmt = require("numfmt");
var import_fx3 = require("@borgar/fx");
...
var import_fx2 = require("@borgar/fx");
var RelativeFormula = class {
  ...
  constructor(formula, anchorCell) {
    ...
    this.relative = (0, import_fx2.translateToR1C1)(formula, anchorCell);
  }
  /** @param {string} offsetCell */
  translate(offsetCell) {
    return (0, import_fx2.translateToA1)(this.relative, offsetCell);
  }
};

// src/handler/cell.ts
var import_simple_xml8 = require("@borgar/simple-xml");
  ...
  if (fNode) {
    ...
    if (f) {
      if (context.options.cellFormulas) {
        ...
        const rc = normalizeFormula((0, import_fx3.translateToR1C1)(f, address), context);
...
// src/handler/worksheet.ts
var import_simple_xml9 = require("@borgar/simple-xml");
function handlerWorksheet(dom, context, rels) {
  ...
  dom.getElementsByTagName("mergeCell").forEach((d) => {
    const ref = attr(d, "ref");
    const { top, left, bottom, right } = (0, import_fx4.parseA1Ref)(ref).range;
    const anchor = (0, import_fx4.stringifyA1Ref)({ range: { top, left } });
    for (let c = left; c <= right; c++) {
      for (let r = top; r <= bottom; r++) {
        context._merged[(0, import_fx4.stringifyA1Ref)({ range: { top: r, left: c } })] = anchor;
      }
    }
  ...
  context._arrayFormula.forEach((arrayRef) => {
    const { top, left, bottom, right } = (0, import_fx4.parseA1Ref)(arrayRef).range;
    for (let r = top; r <= bottom; r++) {
      for (let c = left; c <= right; c++) {
        const ref = (0, import_fx4.stringifyA1Ref)({ range: { top: r, left: c } });
        if (sheet.cells[ref]) {
...
async function convertBinary(buffer, filename, options = DEFAULT_OPTIONS) {
  ...
  const getFile = async (f) => {
    ...
    return fd ? (0, import_simple_xml10.parseXML)(await fd.async("string")) : null;
  };
  const getRels = async (f = "") => {
    const fDir = import_path2.default.dirname(f);
    const fBfn = import_path2.default.basename(f);
    const relsPath = import_path2.default.join(fDir, "_rels", `${fBfn}.rels`);
  ...
  context.filename = import_path2.default.basename(filename);
